...
The following support subs are provided for Module error logging:
<pre><code>
...
Within the BadAss module header, ERRLOGSHEET is a constant set to the string
...
Following is the minimum sequence of calls for issuing an error message to the
message log:
<pre><code>
	dim Doc		As Object
	dim oSel	As Object
...
Explanation: var oRange is set to the current cell selection, which contains the
