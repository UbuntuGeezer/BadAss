#!/bin/bash
echo " ** UpdateBasicFiles.sh.sh out-of-date **";exit 1
...
# 2026-07-22.	wmk.	(automated) UnKillShell to reinstate shell (Lenovo).
#	7/22/26.	wmk.
#
...
# ---------------------
# 7/22/26.	wmk.	(automated) echo,s to printf,s throughout.
...
gitbase=$codebase
printf "%s\n" "**WARNING - all .bas files in EditBas/ folder will be copied over older files"
read -p "  in /Basic folder... Do you wish to proceed (Y/N)? "
yn=${REPLY,,}
if [ "$yn" == "n" ];then
 printf "%s\n" "UpdateBasicFiles abandoned."
 exit
else
 cp -uv $projbase/*.bas $folderbase/Territories/Basic 
 printf "%s\n" "EditBas/*.bas copied to /Basic folder."
