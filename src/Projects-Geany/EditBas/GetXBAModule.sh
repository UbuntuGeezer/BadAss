...
if [ -z "$P1" ];then
 printf "%s\n" "GetXBAModule -h|<xbamodule> missing parameter(s) - abandoned.**"
 read -p "Enter ctrl-c to remain in Terminal: "
 exit 1
fi
cd $gitbase
if ! test -f $gitbase/$P1.xba;then
 printf "%s\n" "** file GitHub/$P1.xba not found for copy - GetXBAModule abandoned.**"
 read -p "Enter ctrl-c to remain in Terminal: "
 exit 1
fi
if [ $gitbase/$P1.xba -ot $gitbase/src/Import/$P1.xba ];then
 printf "%s\n" "**$P1.xba copy skipped - Import/$P1.xba is newer.**"
 ~/sysprocs/LOGMSG "**$ GetXBAModule $P1.xba copy skipped - Import/$P1.xba is newer.**"
else
 cp -uv $gitbase/$P1.xba $gitbase/src/Import/$P1.xba
 printf "%s\n" "$P1.xba copied to Import folder."
 $sp/LOGMSG "  GetXBAModule (BadAss) $P1.xba copied to Import folder."
fi
# end GetXBAModule.sh.
