// macros.gs - macros(BadAss) with Copy of kabAccountingChar GoogleSheets.
//  9/5/24. wmk.  16:37
//
// Modification History.
// ---------------------
// 9/5/24.  wmk.  GetSheetIx added.
...
// Modification History.
// ---------------------
// 9/5/24.  wmk.  use oSheets array to set active sheet.
...
var nAcctIx           // account sheet index
...
var oSheets = [{}];   // sheets array

// code.
try{
  nRetVal = 0;
  oSheets = spreadsheet.getSheets();
...
var lRowIndex;      // insert row index
  ...
  nAcctIx = GetSheetIx(sName);
  oAcctSheet = oSheets[nAcctIx];
//spreadsheet = SpreadsheetApp.getActiveSheet();

// 9/5 try without activating sheet..
...
// GetSheetIx.gs
//-----------------------------------------------
// GetSheetIx - Get sheet index of named sheet.
//    9/5/24. wmk.  10:23
//-----------------------------------------------
function GetSheetIx(psSheet){

// Usage. nSheet = GetSheetIx(sSheet);
//
//  sSheet = sheet name needing index for
//
// Exit. nSheet = sheet index of named sheet (0..n-1)
//          -1 if sheet name not found
//
// Modification History.
// ---------------------
// 9/5/24.  wmk.  original.
//
// Notes. GetSheetIx replaces MoveToSheet from Excel/calc macros.
//
...
try{
  oSheets = spreadsheet.getSheets();
  nSheets = spreadsheet.getNumSheets();
  iRetStatus = -1;
  i = 0;
  bFound = false;
  while(i<nSheets && !bFound){
    sThisName = oSheets[i].getName();
    //sThisName = oThisSheet.getName();
    bFound = (sThisName == psSheet);
    if(bFound){
      iRetStatus = i;
      continue;
    }
  ...
  spreadsheet.toast('in GetSheetIx - unprocessed error.','Error Handler');
  iRetStatus = i;
}   // end catch block

return iRetStatus;

}   // end GetSheetIx   9/5/24.
//**/

// AddCreditTrans.gs
function AddCreditTrans() {
  var spreadsheet = SpreadsheetApp.getActive();
    var oRange = {};
  var sRangeStr;
  var tRangeStr1;
  var tRangeStr2;
  oRange = spreadsheet.getActiveRange();
  spreadsheet.toast('In CopyBelowTrans ' + ' Column = ' + oRange.getColumn() + '\nRow =  ' + oRange.getRow()
  + '\n\n end of message','WhereAmI',5);
  sRangeStr = 'A' + oRange.getRow() + ':' + 'F' + oRange.getRow();
//  tRangeStr1 = 'D' + (oRange.getRow()-1) + ':' + 'D' + (oRange.getRow()-1);
//  tRangeStr2 = 'C' + (oRange.getRow()-1) + ':' + 'C' + (oRange.getRow()-1);
  tRangeStr1 = 'C' + (oRange.getRow()+1) + ':' + 'C' + (oRange.getRow()+1);
  tRangeStr2 = 'D' + (oRange.getRow()+1) + ':' + 'D' + (oRange.getRow()+1);
// spreadsheet.getRange('A1281').activate();
  spreadsheet.getActiveSheet().insertRowsAfter(spreadsheet.getActiveRange().getLastRow(), 1);
  spreadsheet.getActiveRange().offset(spreadsheet.getActiveRange().getNumRows(), 0, 1, spreadsheet.getActiveRange().getNumColumns()).activate();
  spreadsheet.getRange(sRangeStr).copyTo(spreadsheet.getActiveRange(), SpreadsheetApp.CopyPasteType.PASTE_NORMAL, false);
  // now move column C to column D.
  spreadsheet.getRange(tRangeStr1).moveTo(spreadsheet.getRange(tRangeStr2));
} // end AddCreditTrans   8/11/24.
//**/

// AddDebitTrans.gs
//-----------------------------------------------------
// AddDebitTrans - add debit transaction to line above.
//  8/11/24.  wmk.
//-----------------------------------------------------
function AddDebitTrans() {
// Entry. user in active spreadsheet with cell 'A' selected in row to copy
//
// Exit. transaction in selected row copied to preceding line
//    with value negated
//
// Modification History.
// ---------------------
// 8/11/24.   wmk.  original.
//
// Notes. AddDebitTrans checks if the value in the transaction is
// negative (a withdrawal); if so it inverts the value once copied.
  ...
  oRange = spreadsheet.getActiveRange();
  //spreadsheet.toast('In CopyAboveTrans ' + ' Column = ' + oRange.getColumn() 
  //+ '\nRow =  ' + oRange.getRow() + '\n\n end of message','WhereAmI',5);
  sRangeStr = 'A' + (oRange.getRow()+1) + ':' + 'F' + (oRange.getRow()+1)
  tRangeStr1 = 'D' + oRange.getRow() + ':' + 'D' + oRange.getRow();
  tRangeStr2 = 'C' + oRange.getRow() + ':' + 'C' + oRange.getRow();
  tRangeStr3 = 'D' + (oRange.getRow()+1) + ':' + 'D' + (oRange.getRow()+1);
  spreadsheet.getActiveSheet().insertRowsBefore(spreadsheet.getActiveRange().getRow(), 1);
  spreadsheet.getActiveRange().offset(0, 0, 1, spreadsheet.getActiveRange().getNumColumns()).activate();
  spreadsheet.getRange(sRangeStr).copyTo(spreadsheet.getActiveRange(), SpreadsheetApp.CopyPasteType.PASTE_NORMAL, false);
  // now move column D to column C.
  spreadsheet.getRange(tRangeStr1).moveTo(spreadsheet.getRange(tRangeStr2));
  // now negate both values to positive.
  plusval = spreadsheet.getRange(tRangeStr2).getValue();
  if(plusval<0){
    plusval=-plusval
  }
  //spreadsheet.toast('In CopyAboveTrans plusval = ' + plusval,'Negated Value',5);
  spreadsheet.getRange(tRangeStr3).setValue(plusval);
  spreadsheet.getRange(tRangeStr2).setValue(plusval);
}  // end AddDebitTrans   8/11/24.
//**/
